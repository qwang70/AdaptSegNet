...
from model.discriminator_group import FCDiscriminator_group_large
...
RESTART = False
RESTART_FROM = './snapshots/multi_class_250000_seg0.1_adv0.001_bs1_11-17-10-14/'
RESTART_ITER = 40000
...
def generate_snapshot_name(args):
    ...
    snapshot_dir = SNAPSHOT_DIR + 'multi_' + 'class_large' \
        + str(NUM_STEPS) + '_seg{}'.format(args.lambda_seg) \
        + '_adv{}'.format(args.lambda_adv) + '_bs{}'.format(BATCH_SIZE) + \
...
def main():
    ...
    model_D = FCDiscriminator_group_large(num_classes=args.num_classes).to(device)
    ...
    #### model_D are randomly initialized, model is pre-trained ResNet on ImageNet
    else:
        # model parameters
    ...
    for i_iter in range(args.start_steps, args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            loss_seg1 = seg_loss(pred1_interp, labels)
            loss_seg2 = seg_loss(pred2_interp, labels)
            ...
            D_out = model_D(pred_target2)
            # pdb.set_trace()
            ### bce_loss is element-wise, then average all
            loss_adv_target = bce_loss(D_out, torch.FloatTensor(D_out.data.size()).fill_(source_label).to(device))

            loss = args.lambda_adv* loss_adv_target
            loss = loss / args.iter_size
            loss.backward()
        ...
        print(
        'iter = {0:8d}/{1:8d}, loss_seg1 = {2:.3f} loss_seg2 = {3:.3f} loss_adv = {4:.3f} loss_D = {5:.3f}'.format(
            i_iter, args.num_steps, loss_seg_value1, loss_seg_value2, loss_adv_target_value, loss_D_value))

...
def CDAN(input_list, ad_net, random_layer=None, K = 500):
    ...
    if random_layer is None:
        ...
        feature = torch.transpose(torch.transpose(feature.view(B, F, H*W), 0, 1), 0, 1)

        random_matrix_f = torch.randn(C, K, H, W).cuda()
        random_matrix_g = torch.randn(F, K, H, W).cuda()
        random_matrix_f = torch.transpose(torch.transpose(random_matrix_f.view(C, K, H*W), 1, 2), 0, 1)
        random_matrix_g = torch.transpose(torch.transpose(random_matrix_g.view(F, K, H*W), 1, 2), 0, 1)
